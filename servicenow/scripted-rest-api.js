...
/* ─────────────────────────────────────────────────────────
   RESOURCE 23: PUT /auth/charimg
   HTTP Method: PUT  |  Path: /auth/charimg
   Requires Authentication: FALSE
   Body: { charImg: "data:image/jpeg;base64,..." }
   Stores compressed base64 profile picture for the current user.
   NOTE: u_profile_picture field must be String type, max_length 100000.
   ───────────────────────────────────────────────────────── */
(function process(request, response) {
    var _tok = (request.getHeader('Authorization')||'').replace('Bearer ','').trim();
    var _au = new GlideRecord('x_887486_love_app_u_love_auth');
